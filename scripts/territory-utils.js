...
export const UTILITY_CONFIG = {
  'PG&E': { id: 'pge', label: 'PG&E', bundledProvider: 'pge', rateFileKey: 'pge', outputKey: 'pgeTerritory' },
  SCE: { id: 'sce', label: 'SCE', bundledProvider: 'sce', rateFileKey: 'sce', outputKey: 'sceTerritory' },
  TDPUD: { id: 'tdpud', label: 'TDPUD', bundledProvider: 'tdpud', rateFileKey: 'tdpud', outputKey: 'tdpudTerritory' },
  Liberty: { id: 'liberty', label: 'Liberty', bundledProvider: 'liberty', rateFileKey: 'liberty', outputKey: 'libertyTerritory' },
};

export const SUPPORTED_UTILITY_CONFIGS = Object.values(UTILITY_CONFIG);
...
export function buildTerritoryData({ verifiedZips, manualOverrides = [] }) {
  const utilityZips = Object.fromEntries(
    SUPPORTED_UTILITY_CONFIGS.map(config => [config.id, { ...(verifiedZips?.[config.id]?.zips ?? {}) }])
  );
  ...
  for (const override of manualOverrides) {
    ...
    if (override.action === 'assign') {
      ...
      const config = UTILITY_CONFIG[override.utility];
      if (config) {
        for (const otherConfig of SUPPORTED_UTILITY_CONFIGS) {
          if (otherConfig.id !== config.id) {
            delete utilityZips[otherConfig.id][override.zip];
          }
        }
        utilityZips[config.id][override.zip] = override.serviceAreaId;
      }
    }
  ...
  return {
    ...
    sceTerritory: {
      ...
      zips: utilityZips.sce,
    },
    tdpudTerritory: {
      _note: 'TDPUD service territory ZIP codes → service area ID. Generated by scripts/build-territory.js from committed territory source snapshots and documented overrides.',
      zips: utilityZips.tdpud,
    },
    libertyTerritory: {
      _note: 'Liberty Utilities service territory ZIP codes → service area ID. Generated by scripts/build-territory.js from committed territory source snapshots and documented overrides.',
      zips: utilityZips.liberty,
    },
    multiUtilityZips: {
...
export function loadCurrentTerritoryData() {
  return {
    ...
    tdpudTerritory: readJson(PATHS.tdpudTerritory),
    libertyTerritory: readJson(PATHS.libertyTerritory),
...
export function loadVerifiedZips() {
  ...
  if (source.bootstrapFromRuntime) {
    ...
    return {
      ...
      tdpud: { zips: current.tdpudTerritory.zips },
      liberty: { zips: current.libertyTerritory.zips },
...
export function validateTerritoryData({
  ...
  const territoryByUtility = {
    pge: pgeTerritory.zips ?? {},
    sce: sceTerritory.zips ?? {},
    tdpud: tdpudTerritory.zips ?? {},
    liberty: libertyTerritory.zips ?? {},
  };
  const pgeZips = territoryByUtility.pge;
  const sceZips = territoryByUtility.sce;
  const multiZips = multiUtilityZips.zips ?? {};
  const usedServiceAreaIds = new Set();

  for (const config of SUPPORTED_UTILITY_CONFIGS) {
    for (const [zip, serviceAreaId] of Object.entries(territoryByUtility[config.id] ?? {})) {
      usedServiceAreaIds.add(serviceAreaId);
      if (!knownServiceAreaIds.has(serviceAreaId)) {
        addIssue(errors, `${config.label} ZIP ${zip} references unknown serviceAreaId ${serviceAreaId}`, { zip, serviceAreaId });
      }
    }
  }
  ...
  const zipsByAssignment = new Map();
  for (const config of SUPPORTED_UTILITY_CONFIGS) {
    for (const [zip, serviceAreaId] of Object.entries(territoryByUtility[config.id] ?? {})) {
      const assignments = zipsByAssignment.get(zip) ?? [];
      assignments.push({ utility: config.label, serviceAreaId });
      zipsByAssignment.set(zip, assignments);
    }
  }
  for (const [zip, assignments] of zipsByAssignment) {
    if (assignments.length > 1 && !multiZips[zip]) {
      const utilityList = assignments.map(item => item.utility).sort();
      addIssue(
        errors,
        `ZIP ${zip} is assigned to both ${utilityList.join(' and ')} but is missing from multiUtilityZips.json`,
        { zip, assignments }
      );
    }
  }
  ...
  for (const zips of Object.values(territoryByUtility)) {
  ...
  return {
    ...
    stats: {
      ...
      tdpudZipCount: Object.keys(territoryByUtility.tdpud).length,
      libertyZipCount: Object.keys(territoryByUtility.liberty).length,
